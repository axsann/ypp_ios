...
-(void)loadJSON:(NSString *)fileName
{
    ...
    for (int i=0; i<self.cateNameArray.count; i++) {
        ...
        [itemDictArray addObject:fileDict[cateName]];
        ...
        for (int j=0; j<tempArray.count; j++) {
            ...
            // itemの内容をセット
            [item setItemId:tempDict[@"item_id"]
                setItemName:tempDict[@"item_name"]
             setItemImgName:tempDict[@"thumb"]];
...
-(void)loadJSON:(NSData *)jsonData
{
    ...
    //NSData * fileData = [NSData dataWithContentsOfFile:filePath];
    // 辞書データに変換する
    NSDictionary * fileDict = [NSJSONSerialization JSONObjectWithData:jsonData
                                                              options:0
                                                                error:nil];
    // JSONと同じ構造でcateDictを作成する
    // cateDict内には、カテゴリ名をKeyとしてitemArrayが入る
